...
public sealed class YtDlpRunner
{
    ...
    internal static string BuildArguments(RecordingJobContext context)
    {
        ...
        return
            $"--no-part --no-progress --hls-use-mpegts --downloader-args {Quote("ffmpeg_i:-loglevel error")} -f {Quote("bestvideo*+bestaudio/best")} -o {Quote(context.TsPath)}{cookiesArgument} {Quote(context.WatchUrl)}";
    }
