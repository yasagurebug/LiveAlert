...
## 2026/03/15
- Windows 版限定のイースターエッグを追加。`label` が `しののめにこ` / `にこち` / `にこ` のときだけ `assets/nico.png` を埋め込み resource から読み込み、`UpdateLayeredWindow` ベースの透過 TOPMOST ウィンドウを最大30体まで画面上端から降らせるようにした。
- 帯の表示/非表示に連動してイースターエッグを開始・停止し、画像クリックは帯クリックと同じくアラート停止と遷移に接続した。ロック時は帯と同様に停止し、解除後に再表示される。
- Windows 側の判定・移動計算を `NicoEasterEggPhysics` として切り出し、`tests/LiveAlert.Windows.Tests` を追加してラベル判定、反射時の速度更新、座標計算、消滅条件をテストした。
- Windows 版イースターエッグの更新処理を `DispatcherTimer` から `CompositionTarget.Rendering` に切り替え、ウィンドウ移動は `DeferWindowPos` で一括適用するよう変更した。画像向きは `X` 速度が正のときだけ左右反転、`0` 以下は元画像を使うよう修正し、`Y` 方向加速度も従来値の2倍にした。
- Windows 版イースターエッグの `Y` 速度上限を緩和し、画像高さの3倍まで上がるように変更した。
- Windows 版イースターエッグの調整用パラメータを `NicoEasterEggTuning` に集約し、`X` 速度、`Y` 加速度、`Y` 速度上限、発生間隔、跳ね返り確率を1か所で変更できるようにした。
- Windows 版イースターエッグの跳ね返り時 `Y` 速度に反発係数を導入し、維持量を `0.5` にした。

## 未完了
- Windows 設定 UI に色プレビュー以外の入力補助（プリセット、最近使った色など）はまだない。
- Windows 版の実機で、ロック中の音声継続と設定画面レイアウトの最終確認が未実施。
- Windows 版の実機で、`nico.png` の透過表示、クリック判定、マルチモニタ環境での見え方は未確認。

## 次の作業候補
- Windows 設定 UI のログ出力導線。
- 実機でのロック中再生確認。
- Windows 実機でイースターエッグの描画負荷とクリック挙動を確認。
- 色選択 UI の微調整。
